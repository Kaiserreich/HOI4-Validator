...
def check_for_vp_loc():
    ...
    finaldict = actually_check_for_vp_loc(originalpath, newoobdict)
    oobdict, linedict, filedict = create_search_dict(oobdict, linedict, filedict, path, searchstrings, filterstrings,
                                                     thingstripped)
    path = os.path.join(originalpath, 'common', 'technologies')
    oobdict, linedict, filedict = create_search_dict(oobdict, linedict, filedict, path, searchstrings, filterstrings,
                                                     thingstripped)
    oobdict, linedict, filedict = search_effects(oobdict, linedict, filedict, originalpath, searchstrings,
...
def actually_check_for_vp_loc(path, oobdict):
    returndict = oobdict
